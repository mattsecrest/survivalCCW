...
coxph(Surv(t_start, t_stop, death) ~ clone, data = df, weights = weight_cox)
...
boot_cox <- function(data, indices) {
  ...
  cox_ccw <- coxph(Surv(t_start, t_stop, death) ~ clone, data = ccw_df, weights = weight_cox)
  ...
  surv_1 <- survfit(Surv(t_start, t_stop, death) ~ 1L, data = ccw_df[ccw_df$clone == 1, ], weights = weight_cox)

  surv_0 <- survfit(Surv(t_start, t_stop, death) ~ 1L, data = ccw_df[ccw_df$clone == 0, ], weights = weight_cox)
