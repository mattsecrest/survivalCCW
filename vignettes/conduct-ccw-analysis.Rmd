...
boot_cox <- function(data, indices) {
  ...
  hr <- ccw_df |>
    coxph(Surv(t_start, t_stop, death) ~ clone, weights = weight_cox, data = _) |>
    coef() |>
    exp()
  ...
  surv_1 <- ccw_df[ccw_df$clone == 1, ] |>
    survfit(Surv(t_start, t_stop, death) ~ 1L, weights = weight_cox, data = _)

  surv_0 <- ccw_df[ccw_df$clone == 0, ] |>
    survfit(Surv(t_start, t_stop, death) ~ 1L, weights = weight_cox, data = _)
